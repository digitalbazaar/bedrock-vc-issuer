...
export function testIssueWithoutStatus({
  ...
  describe('issue with no status', function() {
    ...
    it('issues twice w/o "credentialId" and no duplicate error', async () => {
      // reuse `credentialId` internal to VC but NOT `options.credentialId` to
      // ensure no storage or uniqueness constraint is applied
