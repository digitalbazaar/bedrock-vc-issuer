...
exports.create = async ({capabilitySet}) => {
  ...
  let record = {
    meta,
    account: database.hash(capabilitySet.account),
    capabilitySet
  };
  try {
...
// get a capability set by instance ID and user ID/account ID
...
exports.update = async ({capabilitySet}) => {
  ...
  const keystoreAgent = await instances.getKeystoreAgent({id: instanceId});
  await revokeCapabilities({instance, keystoreAgent, zcaps: zcapsToRevoke});
