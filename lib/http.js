...
bedrock.events.on('bedrock-express.configure.routes', app => {
  ...
  app.post(
    ...
    asyncHandler(async (req, res) => {
      ...
      const {controller, presentation: vp, configId} = req.body;
  ...
  app.get(
    ...
    asyncHandler(async (req, res) => {
      ...
      if(issuer) {
        const record = await vcIssuer.instances.get({issuer});
        res.json(record);
      } else {
  ...
  app.delete(
    ...
    asyncHandler(async (req, res) => {
      ...
      if(!issuer) {
        // not permitted to delete all instances at this time
        throw new BedrockError(
          'Permission denied.',
          'NotAllowedError', {httpStatusCode: 400, public: true});
      }

      // will throw a 404 if instance does not exist
      ...
      // TODO: also remove configuration before removing instance?
      await vcIssuer.instances.remove({issuer});
      res.status(204).end();
    }));
