  ...
  */
  ...
  app.post(
    ...
    asyncHandler(async (req, res) => {
      ...
      // FIXME: how are we doing tokens?  Headers, in the body like this?
      const {
        issuer,
        credentials,
        token,
      ...
      console.log('PROFILE_AGENT', profileAgentRecord);

      const verifiableCredential = await vcIssuer.issueNew(
        {credentials, profileAgentRecord});
  ...
  app.post(
    ...
    asyncHandler(async (req, res) => {
      ...
      const result = await vcIssuer.issue(
        {actor: null, accountId, instance, credential});
