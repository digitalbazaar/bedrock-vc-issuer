...
bedrock.events.on('bedrock-express.configure.routes', app => {
  ...
  /**
   * /vc-issuer/authenticate
   * Logs in a user into the Issuer management front end app.
   */
  ...
  /**
   * /vc-issuer/issue
   * Issues a VC if the posted data meets business rules, this must include
   * verifying a VP included in the posted data.
   *
   * Authorization required. (Loads a profile agent via Bearer token.)
   */
  app.post(
  ...
  app.post(
    ...
    // profileAgent should be a string here
    asyncHandler(async (req, res) => {
