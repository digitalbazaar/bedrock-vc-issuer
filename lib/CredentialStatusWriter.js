...
export class CredentialStatusWriter {
  ...
  async write({credential, duplicateResult} = {}) {
    ...
    if(listShard) {
      // FIXME: if more than one `TerseStatusListEntry` is present, it may
      // not be possible to detect here which entry needs updating; an
      // alternative approach might be required
      // FIXME: might need to store the credential status(es) written for
      // comparison later
      // FIXME: might be multiple `credentialStatus` results for a given
      // status list config ... handle that here
      ...
      //   `credentialStatus` field:
      // FIXME: `localIndex` is shared across all `credentialStatus` entries
      // for the `statusListConfig`... but multiple `credentialStatus` entries
      // to update for the same `statusListConfig` (and must not touch others
      // that do not match `statusListConfig`)
    ...
    const result = statusLists.map(statusList => {
      return this._upsertStatusEntry({
        credential, statusList, statusListIndex,
  ...
  async exists({statusResult} = {}) {
    assert.array(statusResult, 'statusResult');
    // check every result `meta`'s `credentialStatus.id` for duplicates
    const counts = await Promise.all(statusResult.map(
  ...
  _upsertStatusEntry({
    credential, statusList, statusListIndex, credentialStatus
  }) {
