...
const {default: Bitstring} = require('@digitalbazaar/bitstring');
...
// FIXME: base on status list type and status purpose
// FIXME: bikeshed
const LM_ID = 'urn:vc-issuer:rl2020-lm';
...
export class CredentialStatusWriter {
  constructor({
    slcsBaseUrl, documentLoader, documentStore, issuer, statusPurpose, suite
  } = {}) {
  ...
  async write({credential} = {}) {
    ...
    const {documentStore: {serviceObjectId}} = this;
    ...
    if(listShard) {
      ...
      // FIXME: find matching status using `type` and `statusPurpose`, do not
      // presume there is only one `credentialStatus`
      // FIXME: implement `_findStatusEntry()` helper
    ...
    if(shardQueue.length === 0) {
      ...
      // 2.2. Call listManager.getShard() and store result in the instance.
      const {
        slcsBaseUrl, documentLoader, documentStore, issuer,
        suite
      } = this;
      const listManager = new ListManager({
        // FIXME: once status list 2021 is supported, update `LM_ID` to
        // reflect new status list type and status purpose
        //id: `${LM_ID}-${this.statusPurpose}`,
        id: LM_ID,
        slcsBaseUrl,
        documentLoader,
        documentStore,
    ...
    // FIXME: add helper to create status entry
    // FIXME: support adding multiple credential statuses
