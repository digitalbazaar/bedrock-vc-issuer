...
module.exports = class CredentialStatusWriter {
  ...
  async write({credential} = {}) {
    ...
    if(LIST_SHARDS.length === 0) {
      // 2.1. Create a ListManager instance `listManager`.
      // 2.2. Call listManager.getShard() and store result in the instance.
      const {rlcBaseUrl, credentialsCollection} = this;
      const listManager = new ListManager({rlcBaseUrl, credentialsCollection});
    ...
    const {
      ...
      blockIndex,
      indexAssignmentDoc: {content: {nextListIndex}},
      blockAssignmentDoc: {content: {blockSize}}
    } = this.listShard;
    const revocationListIndex = blockIndex * blockSize + nextListIndex;
