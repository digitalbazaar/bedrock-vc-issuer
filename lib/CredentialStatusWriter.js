...
const MAX_SHARD_QUEUE_SIZE = 100;
...
export class CredentialStatusWriter {
  ...
  _addStatusEntry({credential, statusList, statusListIndex}) {
    ...
    if(type === 'StatusList2021') {
      ...
      // include base URL to apply the `terseStatusListIndex` below to
      credentialStatus.terseStatusListBaseUrl = baseUrl;
      meta.terseStatusListBaseUrl = baseUrl;
    ...
    // do not duplicate terse status list entry
    if(credentialStatus.type === 'TerseBitstringStatusListEntry' &&
      credential.credentialStatus) {
      const existing = Array.isArray(credential.credentialStatus) ?
      ...
      if(existing.some(
        status => (status.type === type &&
          status.terseStatusListBaseUrl === terseStatusListBaseUrl &&
          status.terseStatusListIndex === terseStatusListIndex))) {
