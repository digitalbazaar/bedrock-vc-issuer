  ...
  async write({credential} = {}) {
    ...
    if(listShard) {
      ...
      if(doc.content.slSequence === slSequence &&
        ...
        duplicate, and ensure it is therefore re-sync'd. */
...
function _createListManagerId({statusListConfig}) {
  ...
  // FIXME: use cases:
  // 1. "infinitely" many lists, same purpose
  // 2. "infinitely" many lists, multi purpose
  // 3. finite total block space, same purpose
  // 4. finite total block space, multi purpose
  // 5. adding a new purpose to same / multi purpose after the fact

  // FIXME: a single list manager can work across multiple different status
  // purpose lists, so the status purpose should not be in the ID; need
  // the status list config to specify / deterministically generate an
  // identifier to be used for each wholly independent config (to force
