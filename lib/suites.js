...
export function getSuiteParams({config, suiteName} = {}) {
  ...
  let selectedKeyType = keyType;

  if(suiteName === 'ecdsa-2019') {
    const {zcaps} = config;
    const assertionMethodZcap = Object.keys(zcaps).filter(
      zcap => zcap.startsWith('assertionMethod:'));
    if(assertionMethodZcap.length === 0) {
      throw new Error('No assertionMethod zcap found.');
    }
    // Get the assertion method algorithm specified in config
    const algorithm = assertionMethodZcap[0].split(':')[1];
    if(!keyType.includes(algorithm)) {
      throw new Error(`Unsupported ECDSA key type: ${algorithm}.`);
    }
    selectedKeyType = algorithm;
  }

  let referenceId = `assertionMethod:${selectedKeyType}`;
  ...
  return {zcap, createSuite, referenceId, keyType: selectedKeyType};
}
...
function _createEcdsa2019Suite({signer, keyType}) {
  // remove milliseconds precision
  const date = new Date().toISOString().replace(/\.\d+Z$/, 'Z');
  const cryptosuite = ecdsa2019CryptoSuite;
  cryptosuite.requiredAlgorithm = keyType;
  return new DataIntegrityProof({signer, date, cryptosuite});
}
