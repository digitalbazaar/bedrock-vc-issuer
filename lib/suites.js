...
export function getSuiteParams({config, suiteName, selectedKeyType} = {}) {
  ...
  if(Array.isArray(keyType)) {
    if(!selectedKeyType) {
      throw new Error(`selectedKeyType "${selectedKeyType}" must be specified` +
        `since the suite "${suiteName}" includes multiple key types.`);
    }
    if(!keyType.includes(selectedKeyType)) {
      throw new Error(
        `selectedKeyType "${selectedKeyType}" must be listed in "${keyType}".`);
    }
  } else {
