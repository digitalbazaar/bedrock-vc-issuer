...
bedrock.events.on('bedrock-account.postInsert', async ({account: {id}}) => {
  ...
  if(kek) {
    kek = await controllerKey.getKek({id: kek.id, type: kek.type});
  } else {
    const kekId = `${kmsBaseUrl}/${uuid()}`;
    kek = await controllerKey.generateKey(
      {id: kekId, type: 'kek', kmsModule});
  }
  if(hmac) {
    hmac = await controllerKey.getHmac({id: hmac.id, type: hmac.type});
  } else {
    const hmacId = `${kmsBaseUrl}/${uuid()}`;
    hmac = await controllerKey.generateKey(
      {id: hmacId, type: 'hmac', kmsModule});
  }
  account.kek = {id: kek.id, type: kek.type};
...
exports.getControllerKey = async function({actor, accountId, account = null}) {
  ...
  return ControllerKey.fromSecret({secret, handle: account.id, kmsClient});
};
...
async function _getDataHubDocument({actor, accountId, capability}) {
  ...
  // TODO: ensure required indexes on client
  const invocationSigner = controllerKey;
  return new DataHubDocument({kek, hmac, capability, invocationSigner, client});
}
