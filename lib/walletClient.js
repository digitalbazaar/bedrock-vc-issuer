...
exports.getControllerKey = async function({actor, accountId, account = null}) {
  // FIXME: move controller secret into `instance` ... each instance gets its
  // own secret and controller key
  if(!account) {
...
storage.set = async (
  {actor, accountId, id, capability, content, sequence = 0}) => {
  const edvDoc = await _getEdvDocument({actor, accountId, id, capability});
...
storage.get = async ({actor, accountId, id, capability}) => {
  const doc = await _getEdvDocument({actor, accountId, id, capability});
  return doc.read();
};

async function _getEdvDocument({actor, accountId, id, capability}) {
