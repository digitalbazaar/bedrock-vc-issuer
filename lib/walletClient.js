...
storage.set = async ({actor, accountId, capability, content, sequence = 0}) => {
  const edvDoc = await _getEdvDocument({actor, accountId, capability});
...
storage.get = async ({actor, accountId, capability}) => {
  const doc = await _getEdvDocument({actor, accountId, capability});
  return doc.read();
};

async function _getEdvDocument({actor, accountId, capability}) {
