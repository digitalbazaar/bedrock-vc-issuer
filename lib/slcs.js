...
exports.get = async ({id}) => {
  ...
  const record = await collection.findOne(
    {id: database.hash(id)}, {projection: {_id: 0, credential: 1, meta: 1}});
  if(!record) {
    throw new BedrockError(
 ...
 * @param {string} id - The ID of the RevocationListCredential.
 *
 * @return {Promise<boolean>} resolves to true if stored, false if not.
 */
exports.exists = async ({id}) => {
  assert.string(id, 'id');
  const collection = database.collections['vc-issuer-publishedRlc'];
  const record = await collection.findOne(
    {id: database.hash(id)}, {projection: {_id: 0, id: 1}});
  return !!record;
};
