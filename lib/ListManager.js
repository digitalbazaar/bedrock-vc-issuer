...
export class ListManager {
  constructor({rlcBaseUrl, documentStore, issuer, suite} = {}) {
    assert.string(rlcBaseUrl, 'rlcBaseUrl');
    ...
    while(!this.readActiveList) {
      ...
      if(fullyAssigned.length > 0) {
        ...
        if(blockAssignment.active.length > 0) {
          ...
          readItems = [toUpdate];
          toUpdate.item.revocationListCredential = await this._createRlcId();
  ...
  async _addCapacity({target}) {
    ...
    while(activeListSize < target) {
      ...
      for(const x of newActive) {
        x.revocationListCredential = await this._createRlcId();
        x.rlSequence = ++sn;
      }
      ...
      while(need > 0) {
        blockAssignment.active.push({
          ...
          revocationListCredential: await this._createRlcId(),
          rlSequence: ++sn
        });
        need--;
      }
  ...
  async _createRlcId() {
    return `${this.rlcBaseUrl}/${await generateLocalId()}`
  }
