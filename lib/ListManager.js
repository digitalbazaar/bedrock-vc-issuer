...
import {default as Bitstring} from '@digitalbazaar/bitstring';
...
4. Database (EDV) unique indexes are used to create/find an IAD for each block
   ...
   as well. If the SL sequence number is ever ahead of the value from the LM,
...
1. While active BADs not read:
  ...
  1.2. Read all active BADs.
    ...
    1.2.2. If any BAD has an SL sequence number that is behind the LM,
...
export class ListManager {
  constructor({slcBaseUrl, documentStore, issuer, suite} = {}) {
    assert.string(slcBaseUrl, 'slcBaseUrl');
    assert.object(documentStore, 'documentStore');
    assert.string(issuer, 'issuer');
    assert.object(suite, 'suite');
    if(!slcBaseUrl) {
      throw new TypeError('"slcBaseUrl" must be a non-empty string.');
    }
    if(!issuer) {
      throw new TypeError('"issuer" must be a non-empty string.');
    }
    this.slcBaseUrl = slcBaseUrl;
  ...
  async _createSlcId() {
    return `${this.slcBaseUrl}/${await generateLocalId()}`
  }
  ...
  async _getListManagementDoc() {
    ...
    // FIXME: add timeout
    while(true) {
      const results = await edvClient.find({equals, limit: 1});
      if(results.length === 0) {
        const lmDoc = {
          id: await edvClient.generateId(),
          content: {
            // FIXME: support other types of status lists
            id: LM_DOC_ID,
  ...
  async _ensureStatusListCredentialExists({id, length}) {
    ...
    const verifiableCredential = await issue({credential, suite});
    let doc = {
      id: await edvClient.generateId(),
  ...
  async _init() {
    ...
    // FIXME: determine if setting this is necessary or if the indexes
    // will already have been ensured elsewhere
