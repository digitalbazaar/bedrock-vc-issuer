...
1. The list management document (LMD) is identified by the `indexAllocator`
   ...
   one or more status lists. The state itself tracks two lists: an "active"
   list and an "inactive" list, each with items/elements identifying index
   block assignment documents (BADs). The rationale for having two lists is to
   ...
   fail before completing. Each list item is a 3-tuple, a block assignment
   document (BAD) ID, an SL ID that is exposed in VCs, and an SL list sequence
   number that is kept private. Each SL (including its ID and sequence number)
   is only ever paired with a single BAD, and SL list sequence numbers
