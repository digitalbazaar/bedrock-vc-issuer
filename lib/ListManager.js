...
1. The list management document (LMD) is identified by the `indexAllocator`
   value in a `statusListConfig` and keeps track of index allocation state for
   one or more status lists. The state itself tracks two sets: an "active"
   set and an "inactive" set, each with items/elements identifying index
   block assignment documents (BADs). The rationale for having two sets is to
   allow the system to scale up or down based on demand to assign SL indexes
   and to enable certain non-atomic operations to be continuable, should they
   fail before completing. Each set item is a 3-tuple, a block assignment
   document (BAD) ID, an SL ID that is exposed in VCs, and an SL sequence
   number that is kept private. Each SL (including its ID and sequence number)
   is only ever paired with a single BAD, and SL sequence numbers monotonically
   increase.
...
1. While LMD "active" set size < target:
  // FIXME: simplify to `need === 1 or 0`
  1.1. Set `need` = target - LMD "active" set size. Cap `need` by
    min(1,
        min(need, max configured "active" set size - LMS "active" set size))).
  1.2. If `need` === 0, return `false`, capacity not added.
  1.3. Set `slSequence` to the highest SL sequence number amongst
...
export class ListManager {
  ...
  constructor({
    ...
    this.suite = suite;
    this.lmDoc = null;
    // FIXME: rename to `readActiveSet` (change 'read' if possible to as
  ...
  async _readActiveBlockAssignmentDocs() {
    ...
    while(!this.readActiveList) {
      ...
      let readItems = await Promise.all(blockAssignment.active.map(
      ...
      if(fullyAssigned.length > 0) {
        ...
        blockAssignment.active = blockAssignment.active.filter(
          // remove fully assigned from "active" set
          x => !items.includes(x));
        let toUpdate;
  ...
  async _tryAddCapacity({target}) {
    ...
    while(activeListSize < target) {
      const {blockAssignment} = this.lmDoc.content;
      // 1.1. Set `need` = target - LMD "active" set size. Cap `need` by
      // min(1, min(need, max - LMS "active" set size))).
      ...
      // 1.6. Update the new / newly active item, setting the SL ID and SL
  ...
  _needCapacity() {
    ...
    const {conflicts, readActiveList, statusListConfig} = this;
    if(conflicts < 2) {
      return false;
    }
  ...
  async _readBlockAssignmentDoc({item}) {
    ...
    const pair = {blockAssignmentDoc, item};
    await this._syncBlockAssignmentDocAndItem(pair);
    return pair;
  }
  ...
  async _syncBlockAssignmentDocAndItem(pair) {
    // 1.2.2. If any BAD has an SL sequence number that is behind an LMD item,
    //   CW update it. If conflict, read the BAD.
    if(pair.blockAssignmentDoc.content.slSequence >= pair.item.slSequence) {
      return pair;
    }
    ...
    try {
      ...
      const {content} = pair.blockAssignmentDoc;
      await _resetBlockAssignmentDoc({
        content, slSequence: pair.item.slSequence, statusListConfig
      });
      pair.blockAssignmentDoc = await edvClient.update({
        doc: pair.blockAssignmentDoc
      });
    } catch(e) {
      if(e.name !== 'InvalidStateError') {
        throw e;
      }
      pair.blockAssignmentDoc = await edvClient.get({
        id: pair.blockAssignmentDoc.id
      });
    }
