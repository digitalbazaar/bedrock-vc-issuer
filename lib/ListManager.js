...
export class ListManager {
  constructor({
    id, slcsBaseUrl, documentLoader, documentStore, issuer, statusType, suite
  } = {}) {
    assert.string(id, 'id');
  ...
  async _ensureStatusListCredentialExists({id, length}) {
    ...
    const {statusType, suite, issuer} = this;
    ...
    credential.description =
      `This credential expresses the "${statusType}" status for some other ` +
