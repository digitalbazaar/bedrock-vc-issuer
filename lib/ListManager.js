...
// FIXME: enable configuring limits on the maximum number of active lists, and
// the ability to reuse allocated indexes across multiple statuses, i.e.,
// N-many lists, with index allocation tracked just once, but with one status
// being stored per list at the same index
// FIXME: ensure index distribution continues to meet some reasonable
// packing requirements to promote group privacy; do not fully randomize across
// the whole index space

// total list size is BLOCK_COUNT * BLOCK_SIZE = 131072
const BLOCK_COUNT = 4096;
const BLOCK_SIZE = 32;
...
1. While LMD active list size < target:
  ...
  1.4. Mark up to `need` (1) inactive BADs as active, updating the SL ID and
    SL sequence number (using `++sn`) and decrementing `need`.
  1.5. If `need` > 0, then add `need` (1) more active items as in 1.4.
...
export class ListManager {
  // FIXME: add the status purposes to be tracked (N-many), which will result
  // in 1 list per status purpose (no support for multipurpose lists at this
  // time)
  // FIXME: pass in API for creating / allocating new lists
  // FIXME: add optional `terse` config with maximum index space ... determine
  // FIXME: if this can be part of list creator/allocator API
  constructor({
    id, slcsBaseUrl, documentLoader, documentStore,
    issuer, statusListConfig, suite
  } = {}) {
    assert.string(id, 'id');
    // FIXME: remove `slcsBaseUrl`, `statusListConfig` and put into
    // `ListSource` only
  ...
  async _tryAddCapacity({target}) {
    ...
    while(activeListSize < target) {
      ...
      // 1.3. Mark up to `need` (1) inactive BADs as active, updating the SL ID
      //   and SL sequence number (using `++sn`) and decrement `need` each time.
      const newActive = blockAssignment.inactive.splice(0, need);
      for(const x of newActive) {
        // FIXME: need to generate N-many "next" SLC IDs (one per status
        // purpose) and then create N-many SLCs via the `ListSource` using the
        // old "next" SLC IDs
        x.statusListCredential = await this._createSlcId();
        // note: incrementing `slSequence` will force the related BAD content
        // to be auto-reset before use
        x.slSequence = ++sn;
      }
      need -= newActive.length;
      blockAssignment.active.push(...newActive);
      // 1.4. If `need` > 0, then add `need` (1) more active items as in 1.3.
      if(need > 0) {
  ...
  // FIXME: this should only be called to generate next SLC ID(s) (so rename
  // it appropriately) and those should be stored in the LM doc for the next
  // time a new BAD needs to be created; each time a new one is created the
  // "next" SLC ID(s) must be updated -- so that concurrent processes trying
  // to create a new BAD will use the same values in the event of a failure
  // when communicating with an external status service; how the SLC ID is
  // generated will also depend on "terse" mode: if "terse" is used then the
  // SLC ID will be total-index-based with a list-sized offset integer as
  // a string in its prefix -- and if "terse" is not used, it will be a
  // pseudo-randomly generated string
  async _createSlcId() {
    return `${this.slcsBaseUrl}/${await generateLocalId()}`;
  }
  // FIXME: move to `ListSource`
  async _createSlcIds() {
    return [`${this.slcsBaseUrl}/${await generateLocalId()}`];
  }
      ...
      if(documents.length === 0) {
        ...
        const lmDoc = {
          ...
          content: {
            id: this.id,
            type,
            // FIXME: determine if this needs more than just IDs per element
  ...
  async _readBlockAssignmentDoc({item}) {
    ...
    while(!blockAssignmentDoc) {
      try {
        ...
        // FIXME: there should no longer be a need to ensure that the SLC
        // exists; the BAD item cannot be created without the SLC being
        // created via an external status service

        // doc not found, first ensure status list credential exists
        const listSize = BLOCK_COUNT * BLOCK_SIZE;
        ...
        await _resetBlockAssignmentDoc({
          content: doc.content, slSequence: item.slSequence
        });
        try {
  ...
  async _createNextStatusLists() {
    ...
    // FIXME: move to `ListSource` implementation, passing current SLC IDs
    // and receiving next ones

    // create each SLC in parallel
    await Promise.all(currentSlcIds.map(id => this._createStatusList({id})));

    // generate new SLC IDs for each ID and set them in LM doc
    // FIXME: use `ListSource` to create SLC IDs
    this.lmDoc.content.nextSlcIds = await this._createSlcIds();
  }

  // FIXME: move to `ListSource` and ultimately to status service
  async _createStatusList({id}) {
    const listSize = BLOCK_COUNT * BLOCK_SIZE;
    await this._ensureStatusListCredentialExists({id, length: listSize});
  }
...
async function _resetBlockAssignmentDoc({content, slSequence}) {
