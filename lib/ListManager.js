...
1. While index allocation active cache is not set:
  ...
  1.4. If the cache is empty, call _tryAddCapacity(target=1) and loop to 1.
...
1. Return the result of calling `activeCache.selectShard()`.
  ...
  1.1.2. Call this._preferNewList(conflicts):
    ...
    1.1.2.2. Otherwise, call _tryAddCapacity(target=BADs+1), clear
    ...
    while(!this.activeCache) {
      ...
      if(rotated.length > 0) {
        ...
        if(toUpdate) {
          ...
          conflict). */
          // FIXME: add tests that would skip this update and would cause a
          // conflict that would trigger looping
          try {
