...
// FIXME: replace `uuid` with buid
const {util: {uuid}} = require('bedrock');
const {EdvClient, EdvDocument} = require('edv-client');
const {Bitstring} = require('vc-revocation-list-2020');

// load config defaults
require('./config');

const LM_DOC_ID = 'FIXME: LM doc ID';
...
module.exports = class ListManager {
  constructor({rlcBaseUrl, profileAgentRecord} = {}) {
    ...
    if(!(profileAgentRecord && typeof profileAgentRecord === 'object')) {
      throw new TypeError('"profileAgentRecord" must be an object.');
    }
    this.rlcBaseUrl = rlcBaseUrl;
    // TODO: need to convert `profileAgentRecord` into some API here for
    // reading EDV docs
    this.profileAgentRecord = profileAgentRecord;
    this.lmEdvDoc = null;
    this.lmDoc = null;
      ...
      await Promise.all(readItems.map(async x => {
        if(x.blockAssignmentDoc.content.rlSequence < x.item.rlSequence) {
          try {
            ...
            // TODO: make `blockCount` configurable
            content.blockCount = 1000;
            // TODO: make configurable
            content.blockSize = 100;
  ...
  async _selectShard() {
    ...
    //   it.
    const {blockIndex, blockAssignmentDoc, blockAssignmentEdvDoc, rlSequence} =
      choice;
    ...
    try {
      ...
      if(indexAssignmentDoc.content.rlSequence < rlSequence) {
        ...
        indexAssignmentDoc.content.nextListIndex = 0;
    ...
    // 4. If the IAD's indexes have all been assigned:
    if(indexAssignmentDoc.content.nextListIndex ===
  ...
  async _getListManagementDoc() {
    ...
    while(true) {
      const results = await edvClient.find(
        {equals, capability, invocationSigner});
      if(results.length === 0) {
        // TODO: create LM doc
        const id = await EdvClient.generateId();
        this.lmEdvDoc = this._getEdvDocument({id});
        const lmDoc = {
          id,
          content: {
            id: LM_DOC_ID,
        ...
        try {
          ...
          // FIXME: only ignore duplicate error, throw otherwise
        }
      } else {
        this.lmDoc = results[0];
        this.lmEdvDoc = this._getEdvDocument({id: this.lmDoc.id});
        return;
      }
    }
  }
  ...
  async _readBlockAssignmentDoc({item}) {
    const blockAssignmentEdvDoc = this._getEdvDocument(
    ...
    while(!blockAssignmentDoc) {
      try {
        ...
        // doc not found, lazily create it
        // TODO: make `blockCount` configurable
        ...
        try {
          ...
          // conflict, ignore and loop to read doc
        }
      }
    }
  ...
  async _getIndexAssignmentDoc({
    ...
    if(results.length === 0) {
      // TODO: create doc
      const id = await EdvClient.generateId();
      const indexAssignmentEdvDoc = this._getEdvDocument({id});
      let indexAssignmentDoc = {
        id,
        content: {
          id: `urn:uuid:${uuid()}`,
      ...
      indexAssignmentDoc = await indexAssignmentEdvDoc.write({doc: this.lmDoc});
      return {indexAssignmentDoc, indexAssignmentEdvDoc};
    } else {
      const [indexAssignmentDoc] = results;
      const indexAssignmentEdvDoc = this._getEdvDocument(
  ...
  async _init() {
    ...
    const invocationSigner = await profileAgents.getSigner(
      {profileAgentRecord});
    ...
    this.capability = credentialsEdvZcap;
...
async function _chooseRandom({readActiveList}) {
  const unassignedBlockCount = readActiveList.reduce((count, x) => {
    ...
    count += content.blockCount - content.assignedBlockCount;
  }, 0);
  ...
  for(const x of readActiveList) {
    ...
    if(choice >= unassignedCount) {
      ...
      const bs = await Bitstring.decodeBits({encoded: content.assignedBlocks});
      while(choice > 0) {
...
// TODO: use common function from helpers/utils
