...
export class ListManager {
  ...
  async _tryAddCapacity({cache, target}) {
    ...
    if(add) {
      ...
      try {
        ...
        // if new status lists couldn't be added because of an invalid state
        // or conflict error, then cache is out-of-sync and nothing was added
        if(e.data?.name === 'InvalidStateError' ||
  ...
  async _getListManagementDoc() {
    ...
    while(true) {
      ...
      try {
        this.lmDoc = await edvClient.insert({doc: lmDoc});
        return;
      } catch(e) {
...
async function _chooseRandom({listAssignment}) {
  ...
  // TODO: reuse some code from `IndexAllocationCache.js` here

  // choose one of the unassigned lists
  const unassignedListCount = listCount - assignedListCount;
  let choice = Math.floor(Math.random() * unassignedListCount);
