...
1. While index allocation active cache is not set:
  ...
  1.4. Rotate any active BADs with fully assigned blocks of indexes
    into the "inactive" set:
    1.4.1. If a rotation occurred, the LMD needs to be updated. But
      first, if the cache is now empty, create the next status lists and
      optimistically select a rotated cache record for reuse.
      1.4.1.1. Create new status lists.
      1.4.1.2. Choose a rotated record to leave as active and update its
...
export class ListManager {
  ...
  async _readActiveBlockAssignmentDocs() {
    ...
    while(!this.activeCache) {
      ...
      // FIXME: if cache is empty, try to add capacity
