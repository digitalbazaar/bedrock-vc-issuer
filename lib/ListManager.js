...
export class ListManager {
  ...
  async _tryAddCapacity({cache, target}) {
    ...
    if(add) {
      ...
      newActiveItem.statusListCredential = nextStatusLists[0].id;
  ...
  async _createNextStatusLists() {
    // FIXME: may need to store more than just status list IDs... must store
    // enough information to enable every `credentialStatus` entry to be
    // created (but might be able to leverage `statusListConfig` for this)
  ...
  async _createStatusListMeta() {
    ...
    // `${baseUrl}/<statusPurpose>/<listIndex>`?
    return [listIndex];
  }
  ...
  async _getListManagementDoc() {
    ...
    while(true) {
      ...
      // FIXME: ensure that `listCount` default is set when the status list
      // config was created
      if(terse) {
