...
module.exports = class ListManager {
  constructor({rlcBaseUrl, credentialsCollection} = {}) {
  ...
  async _getListManagementDoc() {
    ...
    while(true) {
      ...
      if(results.length === 0) {
        ...
        this.lmEdvDoc = credentialsCollection.getEdvDocument({id});
        const lmDoc = {
          id,
          content: {
            id: LM_DOC_ID,
        ...
        this.lmEdvDoc = credentialsCollection.getEdvDocument(
          {id: this.lmDoc.id});
        return;
      }
    }
  ...
  async _readBlockAssignmentDoc({item}) {
    const {credentialsCollection} = this;
    const blockAssignmentEdvDoc = credentialsCollection.getEdvDocument(
  ...
  async _getIndexAssignmentDoc({
    ...
    if(results.length === 0) {
      // TODO: create doc
      const id = await EdvClient.generateId();
      const indexAssignmentEdvDoc = credentialsCollection.getEdvDocument({id});
      let indexAssignmentDoc = {
        id,
        content: {
          id: `urn:uuid:${uuid()}`,
      ...
      const indexAssignmentEdvDoc = credentialsCollection.getEdvDocument(
