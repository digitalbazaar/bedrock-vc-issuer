...
1. While index allocation active cache is not set:
  ...
  1.4. If `cache` is empty, call _tryAddCapacity(cache, target=1) and loop to 1.
...
1. Return the result of calling `activeCache.selectShard()`.
  ...
  1.1.2. Call this._preferNewList(conflicts):
    ...
    1.1.2.2. Otherwise, call _tryAddCapacity(activeCache, target=BADs+1), clear
...
export class ListManager {
  ...
  async _readActiveBlockAssignmentDocs() {
    ...
    while(!this.activeCache) {
      ...
      // 1.4. If `cache.isOutOfSync()` loop to 1.
      if(cache.isOutOfSync()) {
        // FIXME: refresh LM doc
        //this.lmDoc = await edvClient.get({id: this.lmDoc.id});
        continue;
      }
  ...
  async _selectShard() {
    ...
    try {
      ...
      // 1.1.2.1. Otherwise, call _tryAddCapacity(activeCache, target=BADs+1),
      //   clear tracked failed attempts, and then return `null`. Note: A
      //   future optimization might check to see if capacity was added and
      //   update BADs accordingly without having to return `null`.
  ...
  async _tryAddCapacity({cache, target}) {
    ...
    // FIXME: combine try/catch for 5. and 6.
    ...
    try {
      ...
      // 5.1. If conflict, read LMD, mark `cache` as out-of-sync, and
      //   return `false` as capacity was not added.
      this.lmDoc = await edvClient.get({id: this.lmDoc.id});
      cache.outOfSync = true;
      return false;
    }

    // 6. If `toUpdate` is set, reset its BAD.
    if(toUpdate) {
      ...
      If we just marked `cache` out-of-sync and returned here instead, it would
      work properly, but would be slower because the active items all have to
      be reread when that is not required if the update to `toUpdate`
      succeeds w/o conflict. */
      ...
      try {
        ...
        // 6.1. If conflict, read LMD and mark `cache` as out-of-sync.
        this.lmDoc = await edvClient.get({id: this.lmDoc.id});
        cache.outOfSync = true;
      }
    }

    // 7. Return `added` to indicate if capacity was added.
    return true;
  }
