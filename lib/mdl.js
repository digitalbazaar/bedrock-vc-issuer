...
export async function envelopeCredential({
  ...
  // FIXME: `devicePublicJwk` should be passed in `options`
  const deviceKeyPair = _generateDeviceKeyPair();
  const devicePublicJwk = deviceKeyPair.publicJwk ?? options.devicePublicJwk;
  ...
  // FIXME: remove me; however, dummy value must be provided as the signing key
  // for `mdocIssuer.sign()`
  const issuerKeyPair = {
    kid: signer.id,
    kty: 'EC',
    x: 'dMYVnW0t8a4GvKqQT8ucXXuKrEgtVskst_7Jef665Tg',
    y: 'dW1ROEY2srPDbbBuGiuabqLYwHMAv3BDkI1ZsQ1GXm8',
    crv: 'P-256',
    d: 'agptxMmYGLl3E_NVnOTnYGHZ6FHrBs9QXPh8x3588fo'
  };
  ...
  const issuerSigned = await mdocIssuer.sign({
    // `signingKey` is an unused dummy value that must be parsable; `signer`
    // API is used instead via `mdocContext`
    signingKey: CoseKey.fromJwk(issuerKeyPair),
...
// FIXME: remove me; device public key must be provided via issue options
function _generateDeviceKeyPair() {
