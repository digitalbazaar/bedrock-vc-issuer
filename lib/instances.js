...
async function _delegateToControllers({id}) {
  ...
  const store = instance.zcaps.find(
    c => c.referenceId === `${id}-configuration`);
  const issue = instance.zcaps.find(
    c => c.referenceId === `${id}-assertionMethod`);
  const authorizations = instance.zcaps.find(
    c => c.referenceId === `${id}-authorizations`);
  ...
  await Promise.all(controller.map(async controller => {
    ...
    // TODO: get did:key from account directly
