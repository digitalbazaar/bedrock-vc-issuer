...
async function validateConfigFn({config} = {}) {
  ...
  try {
    ...
    if(statusListOptions.length > 1) {
      throw new Error(
        'Only one status list option per issuer instance is ' +
        'presently supported.');
    }

    issuer.getSuiteParams({config, suiteName: issueOptions.suiteName});

    for(const {suiteName} of statusListOptions) {
      issuer.getSuiteParams({config, suiteName});
    }
  } catch(error) {
