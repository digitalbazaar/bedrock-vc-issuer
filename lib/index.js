...
bedrock.events.on('bedrock.init', async () => {
  ...
  const service = await createService({
    ...
    validation: {
      ...
      {
        referenceId: 'assertionMethod:P-256',
        required: false
      },
      {
        referenceId: 'assertionMethod:P-384',
        required: false
      }]
    }
  });
...
async function validateConfigFn({config} = {}) {
  try {
    ...
    getSuiteParams({
      config, suiteName: issueOptions.suiteName,
      selectedKeyType: issueOptions.keyType
    });
    for(const {type, statusPurpose, suiteName, keyType} of statusListOptions) {
      ...
      getSuiteParams({config, suiteName, selectedKeyType: keyType});
    }
  } catch(error) {
