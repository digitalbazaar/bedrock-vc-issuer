...
export async function issue({credential, config}) {
  ...
  if(useRevocationList2020) {
    ...
    credentialStatusWriter = new CredentialStatusWriter({
      ...
      issuer,
      suite,
      // FIXME: remove `profileAgent` ... just use issuer instance ID?
      profileAgent
    });
  }
...
export async function revoke({credentialId, config}) {
  ...
  const credentialDoc = await documentStore.get({id: credentialId});
  ...
  // FIXME: use `documentStore.upsert` and `mutator` feature
