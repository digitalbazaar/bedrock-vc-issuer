...
export async function issue({credential, config, suiteName}) {
  ...
  if(useRevocationList2020) {
    ...
    const {issuer, suite} = await _getSLCIssuerAndSuite({config});
    const slcBaseUrl = config.id + bedrock.config['vc-issuer'].routes.slcs;
  ...
  while(!issued) {
    ...
    try {
      ...
      await edvClient.insert({
        ...
        content: verifiableCredential, meta: {type: 'VerifiableCredential'}
      });
      issued = true;
    } catch(e) {
...
export async function revoke({credentialId, config}) {
  ...
  let slcDoc = await documentStore.get(
    {id: revocationListCredential, useCache: false});
...
async function _getDocumentStore({config}) {
  ...
  // FIXME: change to `meta.status` and use string for its value -- future
  // proofed to allow array as well
