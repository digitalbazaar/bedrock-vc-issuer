...
export async function issue({credential, config, options = {}} = {}) {
  ...
  const [documentLoader, documentStore, issuerInfo] = await Promise.all([
    ...
    // only fetch `documentStore` if a status list is configured or
    // `options.credentialId` is given; otherwise, it is not needed
    (statusListOptions.length > 0 || options.credentialId) ?
  ...
  while(!issued) {
    ...
    // if no `options.credentialId` and credential status written, do not store
    // VC; note that this means that VC IDs (`credential.id`) will not be
    // checked for duplicates, this will be the responsibility of clients, etc.
    if(!(credentialStatus?.length > 0 || options.credentialId)) {
      issued = true;
      break;
    }
