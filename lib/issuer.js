...
import {Eddsa2022Suite} from './suites.js';
...
const SUPPORTED_SUITES = new Map([
  ...
  ['Eddsa2022', {
    keyType: 'ed25519', SuiteClass: Eddsa2022Suite
  }]
]);
...
export function getSuiteParams({config, suiteName}) {
  ...
  return {zcap, SuiteClass, referenceId};
}
...
async function _getIssuerAndSuite({
  ...
  const {SuiteClass, referenceId} = getSuiteParams({config, suiteName});
