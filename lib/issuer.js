...
export async function issue({credential, config} = {}) {
  ...
  const {suiteName, keyType} = config.issueOptions;
  const [documentLoader, documentStore, {issuer, suite}] = await Promise.all([
    ...
    _getIssuerAndSuite({config, suiteName, keyType})
  ]);
  ...
  for(const statusListConfig of statusListOptions) {
    const {type = 'RevocationList2020', suiteName, keyType} = statusListConfig;
    ...
    const {issuer, suite} = await _getIssuerAndSuite({
      config, suiteName, keyType
    });
...
export async function setStatus({id, config, statusListConfig, status} = {}) {
  ...
  const [
    ...
    _getIssuerAndSuite({
      ...
      keyType: statusListConfig.keyType
    })
  ]);
...
async function _getIssuerAndSuite({
  ...
  keyType = config.issueOptions.keyType
}) {
  // get suite params for issuing a VC
  const {createSuite, referenceId} = getSuiteParams({
    config, suiteName, selectedKeyType: keyType
  });
  ...
  let suite;
  try {
    suite = createSuite({signer: assertionMethodKey, keyType});
  } catch(e) {
    throw new BedrockError(
      'Unable to create suite.', 'DataError', {
        httpStatusCode: 400,
        public: true
      }, e);
  }
