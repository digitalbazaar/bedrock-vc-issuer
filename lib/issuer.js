...
export async function issue({credential, config, suiteName} = {}) {
  ...
  while(!issued) {
    ...
    try {
      ...
      if(e.name === 'DuplicateError') {
        ...
        // again
        // FIXME: with multiple status writers, this needs to be separated
        // into its own loop; once all writers complete, then issue the VC
        if(credentialStatusWriter &&
  ...
  // FIXME: finish N status writers, not just one
  if(credentialStatusWriter) {
