...
export async function revoke({credentialId, config}) {
  return _setStatus({credentialId, config, statusType: 'revoked'});
}
...
function _getCredentialStatusInfo({credential, statusType}) {
  ...
  if(statusType === 'revoked') {
    // FIXME: add try/catch and try to use status list 2021's
    // getCredentialStatus as well
...
function _getSuiteParams({config, suiteName}) {
...
async function _setStatus({credentialId, config, statusType}) {
  const [
    {documentStore},
    {issuer, suite}
  ] = await Promise.all([
  ...
  while(!slcUpdated) {
    try {
      ...
      // FIXME: use status list and `getStatus()`; should even be backwards
      // compatible with revocation list 2020
      ...
      // delete existing proof and reissue VC
      delete slc.proof;
  ...
  // FIXME: add timeout
  let credentialUpdated = credentialDoc.meta
    .credentialStatus.contains(statusType);
  while(!credentialUpdated) {
    try {
      ...
      credentialUpdated = credentialDoc.meta
        .credentialStatus.contains(statusType);
    }
  }
}
